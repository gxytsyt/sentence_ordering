...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        # print('x.shape', x.shape)  # torch.Size([80, 26, 1024])
        # print('pooled_output.shape', pooled_output.shape)  # torch.Size([80, 1024])
        # print(len(all_encoder_states))  # 13
        # print(len(all_attentions))  # 12
        # for st in all_encoder_states:
        #     print('encoder_states.shape', st.shape)  # torch.Size([80, 26, 1024])
        # for st in all_attentions:
        #     print('all_attentions.shape', st.shape)  # all_attentions.shape torch.Size([80, 16(头数), 26, 26])
...
class HierarchicalAttention(nn.Module):
    ...
    def forward(self, sequence_output, pairs_list, passage_length, pairs_num,
        ...
        # print('sequence_output.shape', sequence_output.shape)  # torch.Size([80, 26, 1024])
        ...
        top_vec_tran_score = self.sentence_tran_2(top_vec_tran_tan).squeeze(-1)  # torch.Size([80, 26])

        # print('top_vec_tran_score.shape', top_vec_tran_score.shape)  # torch.Size([80, 26])
        ...
        # print('selected_masks_all.shape', selected_masks_all.shape)  # torch.Size([80, 2, 26])
        ...
        # print('selected_masks_all_t.shape', selected_masks_all_t.shape)  # torch.Size([80, 26, 2])


        select_score_t = selected_masks_all_t*top_vec_tran_score[:, :, None]
        select_score = select_score_t.permute(0,2,1)

        # print('select_score.shape', select_score.shape)  # torch.Size([80, 2, 26])
        ...
        # print('mix.shape', mix.shape)  # torch.Size([80, 2, 1024])

        mix = mix.reshape(batch, max_pair_num, 2, hidden_size)

        # print('mix.shape2', mix.shape)  # torch.Size([4, 20, 2, 1024])

        final_seq_matriax = torch.zeros(batch, max_num_sen, hidden_size).to(int(cuda[-1]))

        # print('final_seq_matriax.shape', final_seq_matriax.shape)  # torch.Size([4, 5, 1024])
        ...
        # print('cls_score.shape', cls_score.shape)  # torch.Size([80, 2])
        # print('cls_score_matrix_nn.shape', cls_score_matrix_nn.shape)  # torch.Size([4, 5, 5, 2])
        # print('cls_score_batch.shape', cls_score_batch.shape)  # torch.Size([4, 20, 2])
        ...
        # print('cls_output_matrix_nn.shape', cls_output_matrix_nn.shape)  # torch.Size([4, 5, 5, 1024])
        # print('cls_pooled_output_batch.shape', cls_pooled_output_batch.shape)  # torch.Size([4, 20, 1024])


        for idx_i, length in enumerate(passage_length):  # 对于一个batch中每个样本
            current_true_pair_num = pairs_num[idx_i]
            true_pair_list = pairs_list[idx_i][:current_true_pair_num]
            # print('current_true_pair_num', current_true_pair_num)  # 20

            count = [0 for item in range(length)]
            edge_num = int(len(true_pair_list)/length*2)
            # print('edge_num', edge_num)  # 8

            sentence_tensor_i = torch.zeros(length, edge_num, hidden_size).to(int(cuda[-1]))

            # print('sentence_tensor_i.shape', sentence_tensor_i.shape)  # torch.Size([5, 8, 1024])

            for idx_j, pair in enumerate(true_pair_list):

                # mix：torch.Size([4, 20, 2, 1024])
            ...
            sample = sentence_tensor_i  # torch.Size([5, 8, 1024])
            ...
            query_2 = query_2.reshape(passage_num, edge_num)  # torch.Size([5, 8])


            attention_scores = query_2.unsqueeze(dim=1)  # torch.Size([5, 1, 8])

            attention_scores = attention_scores.view(passage_num * 1, edge_num)
            attention_weights = self.softmax(attention_scores)
            attention_weights = attention_weights.view(passage_num, 1, edge_num)  # torch.Size([5, 1, 8])

            # print('attention_weights.shape', attention_weights.shape)  # torch.Size([5, 1, 8])

            mix_new = torch.bmm(attention_weights, sample).squeeze()

            # print('mix_new.shape', mix_new.shape)  # torch.Size([5, 1024])

            final_seq_matriax[idx_i][:length] = mix_new  # torch.Size([4, 5, 1024])

        # final_seq_matriax: 每个句子做完attention后的表示
...
class BartForOrdering(PretrainedBartModel):
    def __init__(self, config: BartConfig, **kwargs):
        super().__init__(config, **kwargs)
        sen_layer_num = 10
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        match_layer_hidden_t, match_layer_attention_t, pair_senvec_tops_t, final_seq_matriax_t, attention_mask_t \
        ...
        return match_layer_hidden_t, attention_mask_t, cls_pooled_output, pair_senvec_tops_t, document_matrix, \
               cls_score, match_layer_attention_t
    ...
    def encode(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        # print('input_ids.shape', input_ids.shape)  # torch.Size([4, 20, 26])
        ...
        # print('input_ids.shape2', input_ids.shape)  # torch.Size([80, 26])
        ...
        top_vec = outputs[0]  # bart最后一层的输出 torch.Size([80, 26, 1024])
        cls_pooled_output = outputs[1]  # bart最后一层cls输出 torch.Size([80, 1024])

        all_layer_hidden = outputs[2]
        attention_score_all = outputs[3]

        # match_layer_hidden_tea = all_layer_hidden
        # match_layer_attention_tea = attention_score_all

        intimate_num_layer = (12 - self.sen_layer_num) + 1

        match_layer_hidden_tea = all_layer_hidden[-intimate_num_layer:]
        match_layer_attention_tea = attention_score_all[-(12 - self.sen_layer_num):]

        # print('len(match_layer_hidden_tea)', len(match_layer_hidden_tea))  # 3
        # print('len(match_layer_attention_tea)', len(match_layer_attention_tea))  # 2
        ...
        cls_score_matrix_nn_his1, cls_score_matrix_nn_his2 \
            = self.two_level_encoder(top_vec, pairs_list, passage_length, pairs_num,
                                     sep_positions, cuda, mask_cls, cls_pooled_output)
        # final_seq_matriax: torch.Size([4, 5, 1024])


        clean_sents_vec = final_seq_matriax * mask_cls[:, :, None].float()

        # print('clean_sents_vec.shape', clean_sents_vec.shape)  # torch.Size([4, 5, 1024])

        mask_cls_tranf = mask_cls.to(dtype=next(self.parameters()).dtype)
        # transformer编码器
        para_matrix = self.encoder(clean_sents_vec, mask_cls_tranf)

        # print('para_matrix.shape', para_matrix.shape)  # torch.Size([4, 5, 1024])

        clean_para_matrix = para_matrix * mask_cls[:, :, None].float()  # 经过transformer编码的句子表示

        # print('clean_para_matrix.shape', clean_para_matrix.shape)  # torch.Size([4, 5, 1024])
        ...
        # print('para_vec.shape', para_vec.shape)  # torch.Size([4, 1024])
        ...
        # print('keyinput.shape', keyinput.shape)  # torch.Size([4, 5, 2048])

        key = self.key_linear(keyinput)
        # print('key.shape', key.shape)  # torch.Size([4, 5, 1024])

        return clean_sents_vec, clean_para_matrix, hcn, key, cls_pooled_output, cls_output_matrix_nn, cls_score, \
               cls_score_matrix_nn, cls_score_matrix_nn_his1, cls_score_matrix_nn_his2, \
               match_layer_hidden_tea, match_layer_attention_tea, pair_senvec_top, final_seq_matriax, attention_mask
